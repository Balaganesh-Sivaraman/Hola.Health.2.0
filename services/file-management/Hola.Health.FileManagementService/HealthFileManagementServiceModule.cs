...
[DependsOn(
    ...
    typeof(HealthFileManagementServiceContractsModule),
...
public class HealthFileManagementServiceModule : AbpModule
{
    ...
    public override async Task OnPreApplicationInitializationAsync(ApplicationInitializationContext context)
    {
        ...
        await scope.ServiceProvider
            .GetRequiredService<FileManagementServiceRuntimeDatabaseMigrator>()
    ...
    private void ConfigureVirtualFileSystem()
    {
        ...
        {
            options.FileSets.AddEmbedded<HealthFileManagementServiceModule>();
        });
    }
    ...
    private void ConfigureSwagger(ServiceConfigurationContext context, IConfiguration configuration)
    {
        if (IsSwaggerEnabled(configuration))
        {
            context.Services.AddAbpSwaggerGenWithOAuth(
                ...
                {
                    {"FileManagementService", "FileManagementService Service API"}
                },
                options =>
                {
                    options.SwaggerDoc("v1", new OpenApiInfo {Title = "FileManagementService API", Version = "v1"});
    ...
    private void ConfigureDatabase(ServiceConfigurationContext context)
    {
        ...
        Configure<AbpDbContextOptions>(options =>
        {
            ...
            options.Configure<FileManagementServiceDbContext>(c =>
            {
                c.UseNpgsql(b =>
                {
    ...
    private void ConfigureDistributedEventBus()
    {
        Configure<AbpDistributedEventBusOptions>(options =>
        {
            ...
            {
                config.UseDbContext<FileManagementServiceDbContext>();
            });
    ...
    private void ConfigureObjectMapper()
    {
        ...
        {
            options.AddMaps<HealthFileManagementServiceModule>(validate: true);
        });
    }
    ...
    private void ConfigureAutoControllers()
    {
        Configure<AbpAspNetCoreMvcOptions>(options =>
        {
            options
                ...
                .Create(typeof(HealthFileManagementServiceModule).Assembly, opts =>
                {
                    opts.RemoteServiceName = "FileManagementService";
                    opts.RootPath = "file-management";
                });
        });
    }
    ...
    {
        options.SwaggerEndpoint("/swagger/v1/swagger.json", "FileManagementService API");
        options.OAuthClientId(configuration["AuthServer:SwaggerClientId"]);
        options.OAuthScopes("FileManagementService");
    }
